...
### Fixed
- **Document IR — revision-skip scan made sound (M15 hardening).** *Internal/experimental.* The `IrReader.Read` perf optimization that skips the `RevisionProcessor` round-trip on revision-free documents had an unsound markup scan: (1) its element set was not a true superset — it omitted `w:tblPrExChange` (which can appear with no other scanned element) and the unconditionally-rewritten `w:delText`/`w:delInstrText`, so documents carrying only those could wrongly skip processing; and (2) it scanned only `MainDocumentPart`, missing header/footer/footnote/endnote/comment-only revisions even though the reader and `RevisionProcessor` consume those parts. The scan now covers the complete element set and every part the reader walks (a `w:numPr`-child `w:ins` was already caught by the existing local-name scan; `w:instrText`/`w:t` stay out, provably masked by their scanned `w:ins` ancestor). New `IrRevisionSkipTests` add two behavioral guards (`tblPrExChange`-only and header-only insertion reads match `RevisionProcessor.AcceptRevisions` output) plus a set-drift guard pinning the scan set to `RevisionProcessor`'s dispatch. Perf is unaffected (corpus ratio still 1.16×, gate ≤ 1.5×); equivalence held at 642/668. No public API change.
