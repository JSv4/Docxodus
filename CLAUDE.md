...
inherited OpenXmlPowerTools core — 2 legacy files (`WmlToHtmlConverter`,
`FormattingAssembler`) carry an explicit `#nullable disable` header,
tracked by issues #648 and #649. They hold back a lot: stripping both takes the
library from its baseline to **1,077** (**963** distinct `CS86xx` sites).
...
**115 warnings**, the test project with **691** (mostly StyleCop `SA1633`/`SA1636` file
