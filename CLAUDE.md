...
**DocxEditor (npm, `npm/src/editor.ts`)** - Framework-agnostic, pure-TypeScript in-browser block editor (the write-side editor counterpart to the read-side projection). Renders a faithful document with `data-anchor` blocks and on commit edits via `DocxSession` then re-renders only the changed block. **It is a single `contenteditable` root** (the editing host), so a native selection spans blocks: body blocks are marked `data-editable="1"` and made `tabindex`-focusable but are NOT their own editing hosts; keydown/blur are root-level; between-block commits come from `selectionchange` — EXCEPT a table cell is its own `contenteditable` host, so its commit comes from the root `blur` when focus leaves the cell (scoped to cells; otherwise an uncommitted cell edit was silently erased by the next structural re-render). **Context-aware Select-All**: `Ctrl/Cmd+A` is intercepted (native `selectAll` mis-selects the first table island) — in the body it selects all body blocks (tables a boundary), inside a table it selects that table's cells (so one font/format applies to the whole table). Tables are a selection boundary (a `contenteditable="false"` island with editable cell paragraphs). **Multi-block selection & actions** (mouse drag / Shift+Click / Shift+Arrow): ribbon commands apply to every selected block, and destructive cross-block edits — type-over, Backspace/Delete, Enter, plain-text paste — are intercepted in `beforeinput` (`editor-input.ts` classifier), routed to `DocxSession`, and committed as **one atomic undo** (client-side undo grouping in `editor.ts`, no engine change). A cross-block delete trims first/middle/last + merges with a seamless join. Selection geometry lives in `editor-selection.ts` (`SelectionModel`); see `docs/architecture/multi_block_selection.md`. `{ paginated: true }` flows blocks into real page boxes via `pagination.ts`. Lossless `save()`. Commands: `format`/`setFontSize`/`setFontFamily`/`setAlignment`/`indent`/`setParagraphStyle`/`toggleList`/`pageBreakBefore` (all apply across a multi-block selection, not just the focused block), `clearParagraphBorders` (remove an HR/paragraph border), `deleteBlock` (remove the active block — inert inside a table or when it is the only editable block), structural `insertHorizontalRule(weight, style, position?)` (`position` = `"above"`|`"below"`, default below; `"above"` puts the rule before the active block — e.g. the S-1 top bar)/`insertTab(alignment="right")` (a real tab stop at the caret — left + right-aligned text on one line, the filing masthead row; inert in a cell)/`insertTable`, table editing `insertTableRow("above"|"below")`/`insertTableColumn("left"|"right")`/`deleteTableRow`/`deleteTableColumn`, `undo`/`redo`, and the `DocxEditor.openBlank(container, exports, options?)` "New document" factory. `setFontSize`/`setFontFamily` cache the last real selection so a focus-stealing combobox/dropdown still applies to a sub-range. `insertTable(rows, cols, options?, position?)` inherits the document font for its cells automatically (resolved from the active block's rendered run, or the nearest non-empty block when the anchor is an empty rule paragraph) so a table matches the surrounding document instead of the blank-doc default; `position` = `"above"`|`"below"` places it relative to a non-empty block (the demo's Above/Below selector drives rules AND tables), and on an empty paragraph it still inserts the table *before* that line so the line becomes the editable paragraph below the table (no stray line above). Single-block format/font/size/alignment ops keep the selection so commands chain (font → bold on one cell needs no re-select); a collapsed caret in a cell + a format command applies to the whole cell. Enter inside a table cell splits the cell paragraph in place (stacked lines); Enter inside an empty horizontal rule does NOT inherit the rule's border; Shift+Enter inserts a real line break (`w:br`). The demo (`examples/editor.html`) ships a visual table grid picker (with an L/C/R cell-alignment selector), a floating table toolbar, an editable font-size combobox, a curated font-family dropdown, an Above/Below rule-position toggle, double-rule and clear-rule buttons, a right-tab button, and a delete-block button; the floating table toolbar docks to a side gutter as a vertical strip when a short table is sandwiched between text (never over the cells). The model-of-record is the live OOXML in `DocxSession`; the IR/anchor system is the addressing overlay (the IR itself is read-only, no IR→OOXML writer). See `docs/architecture/ir_editor_feasibility.md`; the S-1 cover-page feature build is `docs/architecture/s1_smoke_test_features.md`.
...
- Tier B (structural): `InsertParagraph(anchor, Position, markdown)`, `SplitParagraph(anchor, offset)` (splits correctly inside a table cell too — the new `w:p` stays in the `w:tc`; splitting an EMPTY bordered paragraph does NOT copy its `w:pBdr` to the new paragraph, so Enter on a horizontal rule doesn't stack rules; carries the run properties in effect at the split point onto the new paragraph's mark `rPr` so typed continuation keeps bold/italic/font/size — "set the font once and keep typing"; the heading→Normal next-style split and the HR-split-drops-border exceptions still reset), `MergeParagraphs(first, second)`, `InsertHorizontalRule(anchor, Position, ParagraphBorderEdge?)` (empty bottom-bordered paragraph; `Style` supports `single`/`double`/`thick`), `InsertTab(anchor, offset, TabStopAlignment)` (inserts a `w:tab` run at the offset + ensures a `w:tabs/w:tab` stop; `Right` lands at the section's right content margin from the governing `w:sectPr` — left + right-aligned text on one baseline, e.g. a filing masthead's "As filed… / Reg No." row, without a two-column table; de-duped/idempotent), `InsertTable(anchor, Position, rows, cols, TableInsertOptions?)` (borderless option, row-major `CellContents`, `CellAlignment`, per-column `ColumnWidths` (twips), `CellFontFamily` — stamps `w:rFonts` on every cell incl. the paragraph mark so empty cells keep the font when typed into, letting a table match the surrounding document instead of docDefaults; returns created cell anchors; always keeps a trailing `w:p` after the table so an end-of-body table has an editable paragraph below it), and post-insert table editing addressed by a cell-paragraph anchor: `InsertTableRow(cellAnchor, Position)`, `InsertTableColumn(cellAnchor, Position)`, `DeleteTableRow(cellAnchor)`, `DeleteTableColumn(cellAnchor)` (deleting the last row/column removes the table; v1 assumes a rectangular grid, no `w:gridSpan`). Intra-paragraph newlines in the markdown subset (the GFM hard break `"  \n"`) round-trip as a real `w:br`.
