...
| Agent server | `tools/mcp-server/` | .NET 10 console binary (`docxodus-mcp`) speaking newline-delimited JSON-RPC 2.0 (the Model Context Protocol stdio transport) on stdin/stdout. Groups `DocxSessionOps`/`DocxDiffOps` into twelve intent-shaped tools (`docxodus_edit`, `docxodus_format`, `docxodus_table`, `docxodus_comment` (native Word comments), `docxodus_annotate` (overlay), `docxodus_preview` (inline widget render), …) plus open/save/close, for AI agents. Speaks the MCP Apps extension (`io.modelcontextprotocol/ui`): serves a self-contained `ui://docxodus/viewer.html` widget so Claude/ChatGPT render the document inline (`UiResources.cs`), and offers a minimal streamable-HTTP transport (`--http PORT`) for tunnel-based remote-MCP/ChatGPT Apps development. See `docs/architecture/docx_agent_server.md`. |
