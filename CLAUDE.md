...
- Change granularity follows Word Compare's "Show changes at" radio (`ChangeGranularity`, default `Word` like Word's own; `DocxDiffChangeGranularity`). `Character` narrows a change to the differing characters — `colour`→`color` renders retained `colo` + del `u` + retained `r` — in BOTH the markup and the revision list, via the `IrCharacterGranularity` post-pass over every story part. Round-trip-neutral by construction (a character in both texts survives accept and reject wherever it sits), alignment stays word-grained so the edit script is identical either way, and cuts land on grapheme boundaries (never splitting a surrogate pair). Skipped for a pair whose run format, author or date differs, or whose texts are equal (a retargeted hyperlink — emptying both wrappers would erase the revision). Prefix/suffix only, so an interior difference stays coarse; Consolidate is word level regardless (forced). Orthogonal to `RevisionGranularity` (how coarsely the script projects, not how narrowly a change is marked). WmlComparer is word level only
- Table CONTENT/STRUCTURE is compared like Word Compare's default-on "Tables" box (`CompareTables`, default true) - rows, cells and per-cell text, gated at the engine's single `IrTableDiffer.Diff` call site so it reaches nested tables and header/footer/note scopes for free. Off, a content-differing pair emits `EqualBlock` flagged `IrEditOp.Uncompared` (the verbatim emit an unchanged table takes, so the output is always well-formed); the flag is required because `CollectAlignedNumIdPairs` and `IrEditScriptVerifier` both assume an EqualBlock's sides correspond - they now skip it, and the numId remap an uncompared block needs is applied without `w:pPrChange` history so it carries no revision markup. Surviving side is per-scope like `CompareFields` (body -> right, header/footer + note part -> left). A whole table added/removed/MOVED is a block-level op, still reported and reversible; a shell-only pair routes to `FormatOnlyBlock` so table formatting stays tracked and reversible (that axis is `TrackBlockFormatChanges`). Consolidate honors it per reviewer
- Field CODES are compared like Word Compare's default-on "Fields" box (`CompareFields`, default true) — a paragraph's `FieldEnvelopeDigest` (instruction text, `w:fldSimple` vs `w:fldChar` form, inline position, fldChar/fldData scaffolding), gated at all four `StructuralCarrierDiffers` call sites. On, a code change is one reversible whole-paragraph del/ins carrying `w:delInstrText` (the plumbing cannot be sliced into a partial revision) and both round-trip directions hold. Off, a code-only pair is `EqualBlock` and one side's code rides through untracked — body/table/textbox render from the RIGHT (accept ≡ right exact, reject keeps right's code), while a header/footer story or note definition with no ops keeps its LEFT part verbatim (reject ≡ left exact, accept keeps left's code); an uncompared difference is never reversible. Field RESULTS are ordinary text either way, a canonicalized HYPERLINK target stays compared, and Consolidate honors the setting per reviewer. WmlComparer never compares field codes
