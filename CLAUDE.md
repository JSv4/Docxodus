...
| npm/TypeScript | `npm/` | Wrapper around the WASM bridge — `src/index.ts` is the public API, `src/react.ts` is the React hook layer, `src/docxodus.worker.ts`/`worker-proxy.ts` run WASM off the main thread, `src/embed.ts` is the CDN embed entry (`createViewer`/`createEditor` one-call factories; built as `dist/embed.js` ESM + `dist/embed.bundle.js` self-contained + `dist/embed.iife.js` global, WASM path auto-detected from the bundle URL — spec `tests/cdn-embed.spec.ts` exercises it cross-origin via `tests/cors-server.py`). |
