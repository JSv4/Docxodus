...
function safeMetadata(value: unknown, label: string, required = true): string | undefined {
  ...
  const normalized = normalizeFontFamilyName(String(value));
...
function os2LicenseEvidence(
  face: fontkit.Font,
  fileSha256: string,
): { evidence?: FontLicenseEvidence; failure?: string } {
  const os2 = face["OS/2"];
  const flags = os2?.fsType;
  if (!flags) return { failure: "The font has no readable OS/2 embedding-rights table." };
  if (flags.noEmbedding) return { failure: "The OS/2 table forbids font embedding." };
  if (flags.bitmapOnly) return { failure: "The OS/2 table permits bitmap embedding only." };
  if (Number(flags.viewOnly) + Number(flags.editable) > 1) {
    return { failure: "The OS/2 table has contradictory embedding-rights bits." };
  }
...
async function readStableFontFile(
  ...
  try {
    ...
    if (pathAfter.isSymbolicLink() || !pathAfter.isFile()
      ...
      || realPathBefore !== realPathAfter || BigInt(bytes.byteLength) !== after.size) {
...
export async function discoverFontCatalog(
  ...
  for (let directoryIndex = 0; directoryIndex < roots.length; directoryIndex++) {
    const root = roots[directoryIndex];
    const pending = [root];
    const directoryFaces = new Map<string, ConfiguredFontFace>();
    while (pending.length > 0) {
      if (signal?.aborted) throw signal.reason;
      const current = pending.pop()!;
      const entries = await directoryEntries(current, entryCount, limitSnapshot.fontDirectoryEntries);
      ...
      for (const entry of entries) {
        ...
        if (info.isDirectory()) {
          ...
          childDirectories.push(childRealPath);
          continue;
        }
        if (!info.isFile()) {
        ...
        try {
          ...
          const os2License = decoded.format === "ttf" || decoded.format === "otf"
            ? os2LicenseEvidence(parsed, fileSha256)
            : undefined;
          const attested = decoded.format === "woff" || decoded.format === "woff2"
            ? attestedLicenseEvidence(attestationByDigest.get(fileSha256))
            : undefined;
          face = {
            id: `font-${fileSha256}`,
            directoryIndex,
            ...
            codePoints: new Set(parsed.characterSet),
            ...(os2License?.evidence ? { licenseEvidence: os2License.evidence } : {}),
            ...(attested ? { licenseEvidence: attested } : {}),
            ...((decoded.format === "woff" || decoded.format === "woff2") && !attested
              ? { licenseFailure: "A WOFF/WOFF2 font requires an exact embedding-rights attestation." }
              : {}),
            ...(os2License?.failure ? { licenseFailure: os2License.failure } : {}),
          };
        } catch (cause) {
          if (cause instanceof DocxodusExportError) throw cause;
      ...
      childDirectories.sort(compareNames).reverse();
