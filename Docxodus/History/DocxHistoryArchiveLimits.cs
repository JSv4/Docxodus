...
public sealed record DocxHistoryArchiveLimits
{
    ...
    /// <summary>
    /// Graph-blob bytes visited during validation, including repeat reads for effect verification.
    /// ZIP metadata is bounded separately by MaxArchiveBytes, MaxBlobs and MaxManifestBytes.
    /// </summary>
    ...
    public int MaxSnapshotBytes { get; init; } = HistoryBlobIO.DefaultMaxBlobBytes;
    ...
    internal void Validate()
    {
        if (MaxArchiveBytes <= 0 || MaxTotalBlobBytes <= 0 || MaxMetadataBytes <= 0 || MaxValidationBytes <= 0 || MaxExpandedBytes <= 0
            || MaxBlobs <= 0 || MaxEdges <= 0 || MaxManifestBytes <= 0 || MaxBlobBytes <= 0 || MaxSnapshotBytes <= 0 || MaxRecordBytes <= 0)
