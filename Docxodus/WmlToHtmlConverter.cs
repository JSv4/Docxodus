...
namespace Docxodus
{
    ...
    public static class WmlToHtmlConverter
    {
        ...
        #region RenderPageRange - Lazy Loading Support

        /// <summary>
        /// Renders a specific page range from the document for lazy loading.
        ...
        /// <returns>HTML element containing only the requested pages</returns>
        public static XElement RenderPageRange(
            WmlDocument doc,
            ...
            int endPage)
        {
            using (OpenXmlMemoryStreamDocument streamDoc = new OpenXmlMemoryStreamDocument(doc))
            {
                using (WordprocessingDocument document = streamDoc.GetWordprocessingDocument())
                {
                    return RenderPageRange(document, settings, startPage, endPage);
                }
            }
        }
            ...
            int endPage)
        {
            ...
            // Force pagination mode for page range rendering
            var pageRangeSettings = new WmlToHtmlConverterSettings
            {
                PageTitle = settings.PageTitle,
                CssClassPrefix = settings.CssClassPrefix,
                FabricateCssClasses = settings.FabricateCssClasses,
                GeneralCss = settings.GeneralCss,
                AdditionalCss = settings.AdditionalCss,
                RestrictToSupportedLanguages = settings.RestrictToSupportedLanguages,
                RestrictToSupportedNumberingFormats = settings.RestrictToSupportedNumberingFormats,
                ListItemImplementations = settings.ListItemImplementations,
                ImageHandler = settings.ImageHandler,
                RenderTrackedChanges = settings.RenderTrackedChanges,
                RevisionCssClassPrefix = settings.RevisionCssClassPrefix,
                IncludeRevisionMetadata = settings.IncludeRevisionMetadata,
                ShowDeletedContent = settings.ShowDeletedContent,
                AuthorColors = settings.AuthorColors,
                ...
                RenderComments = settings.RenderComments,
                CommentRenderMode = settings.CommentRenderMode,
                CommentCssClassPrefix = settings.CommentCssClassPrefix,
                IncludeCommentMetadata = settings.IncludeCommentMetadata,
                RenderPagination = PaginationMode.Paginated, // Force paginated mode
                PaginationScale = settings.PaginationScale > 0 ? settings.PaginationScale : 1.0,
                PaginationCssClassPrefix = settings.PaginationCssClassPrefix,
                RenderAnnotations = settings.RenderAnnotations,
                AnnotationCssClassPrefix = settings.AnnotationCssClassPrefix,
                AnnotationLabelMode = settings.AnnotationLabelMode,
                IncludeAnnotationMetadata = settings.IncludeAnnotationMetadata
            };

            // Get document metadata to understand structure
            var metadata = GetDocumentMetadata(wordDoc);

            // Calculate page-to-block mapping
            var pageLayout = CalculatePageLayout(wordDoc, metadata);

            // Determine which blocks to render
            var blockRange = GetBlockRangeForPages(pageLayout, startPage, endPage);

            // Render content with block filtering
            return RenderFilteredContent(wordDoc, pageRangeSettings, blockRange, startPage, endPage, pageLayout);
        }

        /// <summary>
        /// Represents the layout of a single estimated page.
        /// </summary>
        private class EstimatedPageLayout
        {
        ...
        /// <summary>
        /// Represents the complete document layout with page assignments.
        /// </summary>
        private class DocumentPageLayout
        {
            public List<EstimatedPageLayout> Pages { get; set; } = new List<EstimatedPageLayout>();
            public int TotalBlocks { get; set; }
        }

        /// <summary>
        /// Calculates estimated page layout by assigning blocks to pages.
        /// Uses heuristics based on paragraph count and section dimensions.
        /// </summary>
        private static DocumentPageLayout CalculatePageLayout(
            WordprocessingDocument wordDoc,
            DocumentMetadata metadata)
        {
            var layout = new DocumentPageLayout();
            ...
            // Get all top-level block elements (paragraphs and tables)
            var blockElements = body.Elements()
                .Where(e => e.Name == W.p || e.Name == W.tbl)
                .ToList();

            layout.TotalBlocks = blockElements.Count;

            if (blockElements.Count == 0)
            {
                // Empty document - create single empty page
                layout.Pages.Add(new EstimatedPageLayout
                {
                    PageNumber = 1,
                    SectionIndex = 0,
                    StartBlockIndex = 0,
            ...
            // Process blocks section by section
            int sectionIndex = 0;
            int blocksInCurrentPage = 0;
            int currentPageStartBlock = 0;

            // Get section info for dimensions
            SectionMetadata currentSection = metadata.Sections.Count > 0
                ? metadata.Sections[0]
                : new SectionMetadata
                {
                    PageWidthPt = 612,
            ...
            // Estimate blocks per page: ~25 paragraphs for letter-size page
            // Adjust based on actual content area
            double baseBlocksPerPage = 25.0;

            foreach (var block in blockElements)
            {
                // Check for section break in paragraph
                if (block.Name == W.p)
                {
                    var pPr = block.Element(W.pPr);
                    var sectPr = pPr?.Element(W.sectPr);
                    if (sectPr != null)
                    {
                        // Section break - close current page
                        if (blocksInCurrentPage > 0)
                        {
                            layout.Pages.Add(new EstimatedPageLayout
                            {
                                PageNumber = currentPageNumber,
                                SectionIndex = sectionIndex,
                                StartBlockIndex = currentPageStartBlock,
                                EndBlockIndex = globalBlockIndex,
                                PageWidthPt = currentSection.PageWidthPt,
                                PageHeightPt = currentSection.PageHeightPt,
                                ContentHeightPt = currentSection.ContentHeightPt
                            });
                            currentPageNumber++;
                            currentPageStartBlock = globalBlockIndex + 1;
                            blocksInCurrentPage = 0;
                        }

                        // Move to next section
                        sectionIndex++;
                        if (sectionIndex < metadata.Sections.Count)
                        {
                            currentSection = metadata.Sections[sectionIndex];
                        }
                    }

                    // Check for explicit page break
                    bool hasPageBreak = pPr?.Element(W.pageBreakBefore) != null ||
                        block.Descendants(W.br).Any(br => (string)br.Attribute(W.type) == "page");

                    if (hasPageBreak && blocksInCurrentPage > 0)
                    {
                        // Page break - close current page
                        layout.Pages.Add(new EstimatedPageLayout
                        {
                            PageNumber = currentPageNumber,
                            SectionIndex = sectionIndex,
                            StartBlockIndex = currentPageStartBlock,
                            EndBlockIndex = globalBlockIndex - 1,
                            PageWidthPt = currentSection.PageWidthPt,
                            PageHeightPt = currentSection.PageHeightPt,
                            ContentHeightPt = currentSection.ContentHeightPt
                        });
                        currentPageNumber++;
                        currentPageStartBlock = globalBlockIndex;
                        blocksInCurrentPage = 0;
                    }
                }

                // Count this block
                int blockWeight = block.Name == W.tbl ? 3 : 1; // Tables count as ~3 blocks
                blocksInCurrentPage += blockWeight;

                // Adjust blocks per page based on section dimensions
                double pageAreaRatio = (currentSection.ContentWidthPt * currentSection.ContentHeightPt) / (468.0 * 648.0);
                double adjustedBlocksPerPage = baseBlocksPerPage * Math.Max(0.5, pageAreaRatio);

                // Check if we need a new page
                if (blocksInCurrentPage >= adjustedBlocksPerPage)
                {
                    layout.Pages.Add(new EstimatedPageLayout
                    {
                        PageNumber = currentPageNumber,
            ...
            if (blocksInCurrentPage > 0 || currentPageStartBlock < blockElements.Count)
            {
                layout.Pages.Add(new EstimatedPageLayout
                {
                    PageNumber = currentPageNumber,
                    SectionIndex = sectionIndex,
                    StartBlockIndex = currentPageStartBlock,
                    EndBlockIndex = blockElements.Count - 1,
                    PageWidthPt = currentSection.PageWidthPt,
                    PageHeightPt = currentSection.PageHeightPt,
                    ContentHeightPt = currentSection.ContentHeightPt
                });
            ...
            if (layout.Pages.Count == 0)
            {
                layout.Pages.Add(new EstimatedPageLayout
                {
                    ...
                    SectionIndex = 0,
                    StartBlockIndex = 0,
                    EndBlockIndex = Math.Max(0, blockElements.Count - 1),
                    PageWidthPt = currentSection.PageWidthPt,
                    PageHeightPt = currentSection.PageHeightPt,
                    ContentHeightPt = currentSection.ContentHeightPt
                });
        ...
        /// <summary>
        /// Gets the range of block indices that should be rendered for the given page range.
        /// </summary>
        private static (int startBlock, int endBlock) GetBlockRangeForPages(
            DocumentPageLayout layout,
            int startPage,
            int endPage)
        {
            // Clamp page numbers
            startPage = Math.Max(1, Math.Min(startPage, layout.Pages.Count));
            endPage = Math.Max(startPage, Math.Min(endPage, layout.Pages.Count));

            // Find the block range
            int startBlock = int.MaxValue;
            int endBlock = int.MinValue;

            foreach (var page in layout.Pages)
            {
                if (page.PageNumber >= startPage && page.PageNumber <= endPage)
                {
                    startBlock = Math.Min(startBlock, page.StartBlockIndex);
                    endBlock = Math.Max(endBlock, page.EndBlockIndex);
                }
            }

            if (startBlock == int.MaxValue)
            {
                // No matching pages found - return full document
                startBlock = 0;
                endBlock = layout.TotalBlocks - 1;
            }
        ...
        /// <summary>
        /// Renders document content filtered to only include specified blocks.
        /// </summary>
        private static XElement RenderFilteredContent(
            WordprocessingDocument wordDoc,
            WmlToHtmlConverterSettings settings,
            (int startBlock, int endBlock) blockRange,
            int startPage,
            int endPage,
            DocumentPageLayout pageLayout)
        {
            // Only accept revisions if NOT rendering tracked changes
            ...
            XElement body = rootElement.Element(W.body);

            if (body == null)
            {
                // Return minimal HTML for empty document
                return CreateMinimalHtml(settings);
            }

            // Annotate for sections (needed for proper section handling)
            AnnotateForSections(wordDoc);

            // Set up comment and annotation tracking
            var commentTracker = new CommentTracker();
            ...
            // Create page range filter
            var pageRangeFilter = new PageRangeFilter
            {
                StartBlockIndex = blockRange.startBlock,
                EndBlockIndex = blockRange.endBlock,
                StartPage = startPage,
                EndPage = endPage,
            ...
            var bodyContent = BuildFilteredBodyContent(wordDoc, settings, body, blockRange, pageRangeFilter);

            // Build CSS
            var cssClasses = new Dictionary<string, string>();
            var usedCssClassNames = new HashSet<string>();
            rootElement.AddAnnotation(cssClasses);
            rootElement.AddAnnotation(usedCssClassNames);

            // Create complete HTML structure
            var xhtml = new XElement(Xhtml.html,
                new XElement(Xhtml.head,
                    new XElement(Xhtml.meta, new XAttribute("charset", "UTF-8")),
                    settings.PageTitle != null ? new XElement(Xhtml.title, settings.PageTitle) : null,
                    settings.GeneralCss != null ? new XElement(Xhtml.style,
                        new XAttribute("type", "text/css"),
                        settings.GeneralCss) : null,
                    settings.AdditionalCss != null ? new XElement(Xhtml.style,
                        new XAttribute("type", "text/css"),
                        settings.AdditionalCss) : null),
                bodyContent);

            // Create usedCssClassNames annotation for ReifyStylesAndClasses
            xhtml.AddAnnotation(cssClasses);
            xhtml.AddAnnotation(usedCssClassNames);

            ReifyStylesAndClasses(settings, xhtml);

            return xhtml;
        }

        /// <summary>
        /// Builds the body content with filtered blocks for page range rendering.
        /// </summary>
        private static XElement BuildFilteredBodyContent(
            WordprocessingDocument wordDoc,
            WmlToHtmlConverterSettings settings,
            XElement body,
            (int startBlock, int endBlock) blockRange,
            PageRangeFilter filter)
        {
            var prefix = settings.PaginationCssClassPrefix ?? "page-";
            var bodyContent = new List<object>();

            // Get all block elements
            var allBlocks = body.Elements()
                .Where(e => e.Name == W.p || e.Name == W.tbl)
                .ToList();

            // Filter to requested range
            var filteredBlocks = new List<XElement>();
            for (int i = 0; i < allBlocks.Count; i++)
            {
                if (i >= blockRange.startBlock && i <= blockRange.endBlock)
                {
                    filteredBlocks.Add(allBlocks[i]);
                }
            }

            // Get section properties
            var sectPrs = body.Elements(W.sectPr).ToList();

            // Transform each block and add block index
            int blockIndex = blockRange.startBlock;
            var transformedBlocks = new List<object>();

            foreach (var block in filteredBlocks)
            {
                var transformed = ConvertToHtmlTransform(wordDoc, settings, block, false, 0m);
                if (transformed is XElement elem)
                {
                    elem.SetAttributeValue("data-block-index", blockIndex);
                }
                else if (transformed is IEnumerable<object> items)
                {
                    foreach (var item in items)
                    {
                        if (item is XElement itemElem)
                        {
                            itemElem.SetAttributeValue("data-block-index", blockIndex);
                        }
                    }
                }
            ...
            // Get first section dimensions for the container
            var firstPage = filter.PageLayout.Pages.FirstOrDefault();
            var pageWidth = firstPage?.PageWidthPt ?? 612;
            var pageHeight = firstPage?.PageHeightPt ?? 792;

            // Create section div with dimensions
            var sectionDiv = new XElement(Xhtml.div,
                new XAttribute("class", prefix + "section"),
                new XAttribute("data-page-width", pageWidth.ToString("F1", NumberFormatInfo.InvariantInfo)),
                new XAttribute("data-page-height", pageHeight.ToString("F1", NumberFormatInfo.InvariantInfo)),
                transformedBlocks);

            // Create page container
            var pageContainer = new XElement(Xhtml.div,
                new XAttribute("id", "pagination-container"),
                new XAttribute("class", prefix + "container"),
                new XAttribute("data-start-page", filter.StartPage),
                new XAttribute("data-end-page", filter.EndPage),
                new XAttribute("data-total-pages", filter.PageLayout.Pages.Count),
                new XAttribute("data-start-block", filter.StartBlockIndex),
                new XAttribute("data-end-block", filter.EndBlockIndex),
                sectionDiv);

            bodyContent.Add(pageContainer);

            // Add header/footer registry if enabled
            if (settings.RenderHeadersAndFooters)
            {
                var hfRegistry = RenderPaginatedHeaderFooterRegistry(wordDoc, settings, body.Parent ?? body);
                if (hfRegistry != null)
                {
                    var stagingDiv = new XElement(Xhtml.div,
                        new XAttribute("id", "pagination-staging"),
                        new XAttribute("class", prefix + "staging"),
                        new XAttribute("style", "display: none;"),
                        hfRegistry);
                    bodyContent.Insert(0, stagingDiv);
                }
            }

            // Add footnote registry if enabled
            if (settings.RenderFootnotesAndEndnotes)
            {
                ...
                {
                    var existingStaging = bodyContent.OfType<XElement>()
                        .FirstOrDefault(e => (string)e.Attribute("id") == "pagination-staging");

                    if (existingStaging != null)
                    {
                        existingStaging.Add(footnoteRegistry);
                    }
                    else
                    {
                        var stagingDiv = new XElement(Xhtml.div,
        ...
        /// <summary>
        /// Marker class for page range filtering during transform.
        /// </summary>
        private class PageRangeFilter
        {
            public int StartBlockIndex { get; set; }
            public int EndBlockIndex { get; set; }
            public int StartPage { get; set; }
            public int EndPage { get; set; }
            public DocumentPageLayout PageLayout { get; set; }
        }

        /// <summary>
        /// Creates minimal HTML structure for empty or invalid documents.
        /// </summary>
        private static XElement CreateMinimalHtml(WmlToHtmlConverterSettings settings)
        {
            var prefix = settings.PaginationCssClassPrefix ?? "page-";
            return new XElement(Xhtml.html,
                ...
                new XElement(Xhtml.body,
                    new XElement(Xhtml.div,
                        new XAttribute("class", prefix + "container"),
                        new XAttribute("data-total-pages", "0"),
                        new XAttribute("data-rendered-pages", "0"))));
        }

        /// <summary>
        /// Modified HTML transform that filters content based on block range.
        /// </summary>
        private static object ConvertToHtmlTransformFiltered(
            WordprocessingDocument wordDoc,
            WmlToHtmlConverterSettings settings,
            XElement element,
            ...
            (int startBlock, int endBlock) blockRange)
        {
            // Handle body element specially to filter blocks
            if (element.Name == W.body)
            {
                var prefix = settings.PaginationCssClassPrefix ?? "page-";
                var pageRangeFilter = element.Document?.Root?.Annotation<PageRangeFilter>();

                // Get block elements
                var allBlocks = element.Elements()
                    .Where(e => e.Name == W.p || e.Name == W.tbl)
                    .ToList();
                ...
                for (int i = 0; i < allBlocks.Count; i++)
                {
                    if (i >= blockRange.startBlock && i <= blockRange.endBlock)
                    {
                        ...
                        // Add block index annotation for client-side use
                        block.SetAttributeValue("data-block-index", i);
                        filteredBlocks.Add(block);
                    }
                }

                // Also include sectPr elements that follow filtered blocks
                var sectPrs = element.Elements(W.sectPr).ToList();

                // Create filtered body with only the requested blocks
                var filteredBody = new XElement(W.body,
                    filteredBlocks,
                    sectPrs);

                // Copy annotations from original body
                foreach (var annotation in element.Annotations<object>())
                {
                    filteredBody.AddAnnotation(annotation);
                }

                // Now transform the filtered body
                return TransformBodyForPageRange(wordDoc, settings, filteredBody, pageRangeFilter);
            }

            // For non-body elements, use the standard transform
            return ConvertToHtmlTransform(wordDoc, settings, element, suppressTrailingWhiteSpace, currentMarginLeft);
        }

        /// <summary>
        /// Transforms a filtered body element for page range rendering.
        /// </summary>
        private static XElement TransformBodyForPageRange(
            WordprocessingDocument wordDoc,
            WmlToHtmlConverterSettings settings,
            XElement filteredBody,
            PageRangeFilter filter)
        {
            var prefix = settings.PaginationCssClassPrefix ?? "page-";
            var bodyContent = new List<object>();

            // Create section divs for the filtered content
            var mainContent = CreateFilteredSectionDivs(wordDoc, settings, filteredBody, filter);

            // Wrap in page range container
            var pageContainer = new XElement(Xhtml.div,
                new XAttribute("id", "pagination-container"),
                new XAttribute("class", prefix + "container"),
                new XAttribute("data-start-page", filter.StartPage),
            ...
            // Add header/footer registry if enabled (needed for client-side rendering)
            if (settings.RenderHeadersAndFooters)
            {
                var hfRegistry = RenderPaginatedHeaderFooterRegistry(wordDoc, settings, filteredBody.Parent ?? filteredBody);
                if (hfRegistry != null)
                {
                    var stagingDiv = new XElement(Xhtml.div,
                        new XAttribute("id", "pagination-staging"),
        ...
        /// <summary>
        /// Creates section divs for filtered content with block index tracking.
        /// </summary>
        private static object CreateFilteredSectionDivs(
            WordprocessingDocument wordDoc,
            WmlToHtmlConverterSettings settings,
            XElement body,
            PageRangeFilter filter)
        {
            ...
            // Group filtered elements by section
            ...
            if (elements.Count == 0)
            {
                return new XElement(Xhtml.div,
                    new XAttribute("class", prefix + "empty"),
                    new XAttribute("data-message", "No content in requested page range"));
            }

            var groupedIntoDivs = elements
                .GroupAdjacent(e =>
                {
            ...
            int blockIndex = filter.StartBlockIndex;
            var divList = groupedIntoDivs
                .Select(g =>
                {
                    ...
                    var transformedContent = g.Select(block =>
                    {
                        var transformed = ConvertToHtmlTransform(wordDoc, settings, block, false, 0m);
                        if (transformed is XElement elem)
                        {
