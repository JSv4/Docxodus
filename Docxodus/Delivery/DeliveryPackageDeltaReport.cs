...
public sealed record DeliveryPackageDeltaReport
{
    ...
    /// <summary>Same ceiling as the deliverable verifier's MaxReportedDeltaChanges default.</summary>
    private const int MaxReportedChanges = 25_000;
        ...
        PackageManifest final)
    {
        ...
        var delta = PackageDelta.Compare(baseline, final, MaxReportedChanges);
        if (!delta.Complete)
        {
            throw new InvalidDataException(
                $"The package delta exceeded {MaxReportedChanges} changes; a truncated delta "
                + "artifact cannot claim completeness.");
        }
