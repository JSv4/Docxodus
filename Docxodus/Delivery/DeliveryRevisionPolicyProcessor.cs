...
public static class DeliveryRevisionPolicyProcessor
{
    ...
    /// <summary>Finding budget for the classification pass; sized well above the element cap.</summary>
    private const int MaxClassificationFindings = 10_000;
        ...
        PackageManifestOptions? options = null)
    {
        ...
        // The verifier's classifier is three-document (baseline, intended final, redline); policy
        // derivation has no intended-final document yet, so an empty final inventory reduces it to
        // the two-document split this processor needs: baseline-owned versus generated.
        var classifications = RedlineReversibilityVerifier.Classify(
            baselineEntries,
            Array.Empty<RevisionListEntry>(),
            editedEntries,
            classificationFindings,
            new RedlineReversibilityVerifier.ProofFindingBudget(MaxClassificationFindings));
        ...
        DeliveryRevisionPolicy action)
    {
        ...
        var currentIdentities = currentEntries
            ...
            .ToArray();
        var exact = currentIdentities.Where(item => IdentityEquivalent(requested, item))
            .ToArray();
        if (exact.Length == 0)
        {
    ...
    /// <summary>
    /// Strict full-field revision identity. The verifier's ownership comparison is deliberately
    /// looser (part/type/author/date/resolution); policy resolution must not treat a revision whose
    /// content or anchors changed mid-derivation as the one the caller requested, so every identity
    /// field participates here.
    /// </summary>
