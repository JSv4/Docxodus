...
| Tables | — | ~ compared, correctness issues remain; no toggle on `WmlComparer`. `DocxDiffSettings.CompareTables` is the toggle on `DocxDiff` (off ⇒ the right table rides through untracked) |
