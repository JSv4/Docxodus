...
- **Produced-markup parity:** floor 39 fixtures round-trip clean (accept ≡ right, reject ≡ left, schema-valid); the round-trip allowlist is **2 fixtures** (WC022 adjacent-empty-paragraph alignment ordering, WC-BodyBookmarks endnote→footnote note-store conversion), both with established reader/aligner-level root causes (none a renderer-markup fault). M2.6 Task 1 closed the WC034 foot+end note-renumber pair via `IrMarkupRenderer.RenumberNoteIds` (a body-reference-order note-id renumber/reorder output pass mirroring the oracle's `ChangeFootnoteEndnoteReferencesToUniqueRange`).
