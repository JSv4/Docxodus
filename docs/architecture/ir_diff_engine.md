...
- **Wiring.** `DocxCompare.ToDocxDiffSettings` sets it on the engine-selector path (CLI `--engine=docxdiff` / WASM / npm), so the selector reproduces Word's preservation. Consolidate does not preserve in v1 (the composite renderer builds no map), though Preserve still suppresses the pre-accept there when both flags are set.
