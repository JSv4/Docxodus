...
| M2.6 | Sub-paragraph split/merge alignment (Phase-2 follow-on) | post-M2.5 | **COMPLETE (2026-06-12)** — first-class 1:N split / N:1 merge SHIPPED, closing the last two GetRevisions deviations: **scoreboard 179/179 GENUINE, deviation catalog EMPTY (asserted), genuine-pass ratchet 177→179; markup allowlist 4→1** (WC022 closed in T2; only the oracle-crashes WC-BodyBookmarks fixture remains anywhere). Op model: `SplitBlock`/`MergeBlock` kinds + `SplitMergeAnchors`/`SegmentDiffs` trailing fields (additive; JSON byte-identical for 1:1 scripts; N:M rejected by the load-bearing `AssertSplitMergePairing`). Detection: side-parameterized containment scan in `FillOneGap` (after similarity, before the 1×1 rule; LCS coverage ≥0.90 / slack ≤0.34 — corpus-SWEPT with a pinned plateau+margin gate; zero-match edge trim as the false-positive guard; may PROMOTE a same-gap Modified pairing; Unchanged/FormatOnly never candidates — WC022 invariant preserved and re-tested; O(1) content-count prefilter keeps the 200×200 adversarial bound). Segmentation: per-member Myers re-diff over LCS-assigned slices — the partition invariant makes boundaries implicit (F3.3). Surfaces: apply-verifier per-member tuples + cell-path anchor-order proof (F3.1/F3.2), Fine per-segment + per-mark account, compat = the oracle's one-coalesced-insert account (WC-1830→2, WC-1450→7), anchored-split markup via `MarkParagraphMark` with accept≡right/reject≡left proven on both fixtures + synthetics; `SplitParagraph`/`MergeParagraphs` fuzz mutations (own-oracle 1000 seeds; excluded from the differential class — the engines frame a clean split differently by construction). Bonus: the reshuffled fuzz seeds exposed + fixed a PRE-EXISTING reject-order bug (deletions anchored to moved-away lefts). `IrDiffSettings.DetectSplitMerge` default ON (false = strict 1:1 semantics). Full MUST-FIX closure + spec deltas (incl. the §3.2 'Second'-cell turning out NOT to be a split) in the spec's IMPLEMENTATION OUTCOME section; the canonical algorithm description is `docs/architecture/ir_diff_engine.md`. Earlier (same day): SKETCHED/DEFERRED — M2.5 Task 2 established that WC-1450 AND WC-1830 share ONE root cause: a **1:N paragraph SPLIT** (one before-paragraph's content migrates across two after-paragraphs). The oracle's flat atom LCS credits both split halves as Equal against the single before-paragraph (a 1:2 match); the IR's `IrEditOp` is strictly 1:1, so it keeps one half and surfaces the other as a whole-paragraph revision (+1 each). PROVED not closable in the 1:1 model and not render-coalescible (interleaved Insert/Insert/Modify/Delete ops; re-pairing the other half is symmetric). Correct fix = engine-level 1:N split/merge op + apply-verifier / markup-renderer / JSON / fuzzer ripple — a real capability, not a patch; explicitly NOT shipped under the M2.5 timebox. Design sketch (detection in gap fill via in-order containment of `bag(L)` by the union of an adjacent right-block run; `IrSplitBlockOp`/`IrMergeBlockOp` representation; ripple contract) in [`2026-06-12-subparagraph-split-merge-design.md`](./2026-06-12-subparagraph-split-merge-design.md). WC-1450/1830 retained as deviations with this sketch referenced (floors unchanged 179/174). The WC-1450 catalog description was also corrected (the old "two identical paragraphs anchor ambiguity" was stale — it is the same split). |
