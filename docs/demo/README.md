...
|---|---|---|
| `revisions` | `docxDiffGetRevisions` | ~6.5 diffs/s, p50 **152 ms** |
| `redline` | `docxDiffCompareProducts` → redline package + revisions | ~5.4 diffs/s, p50 **181 ms** |
| `full + HTML` | the above, then `convertDocxToHtml` with tracked changes | ~2.8 diffs/s, p50 **352 ms** |

Against a mutation costing ~2–3 ms through the same MCP endpoint, that is **60× to
154×**, and the panel reports the ratio it just measured rather than the one
...
both products measured **126 ms against 196 ms** for the two calls separately —
...
These figures track the engine, and have moved twice without a line of demo code
changing — which is the argument for a panel that measures rather than a page that
quotes. #616 roughly halved `docxDiffCompare` on this document (278 ms → 153 ms)
by removing a read amplification. #626 then gated the compatibility normalizer on
a streaming scan instead of a full parse, and the two deeper rows above came down
10–18% (`redline` 206 → 181, `full` 422 → 352) while `revisions` stayed put inside
the noise band. The saving growing with pipeline depth is what you would expect
from a load-path change — the deeper the pipeline, the more packages it opens —
though the panel measures the total, not the attribution.
