...
// Ordered by measured ink coverage in the shipped Canvas Mono font, with
// synthetic bold as used in the document. Equal character-index steps are
// NOT equal brightness steps: the gap between ';' and 'i' is substantial.
export const ASCII_RAMP = ' .,:;ris235hSXGA&9HB#M@';
const coverage = [0, 47, 76, 93, 122, 177, 243, 246, 294, 302, 311, 316,
  319, 330, 338, 342, 364, 366, 369, 415, 417, 458, 461];
const tones = Array.from({ length: 256 }, (_, value) => {
  const target = Math.pow(value / 255, .8) * 461;
  let best = 0;
...
// Every pixel keeps its own glyph/tone; merging ink does not smear a weapon,
// numeral or wall edge. Bright red/grey boundaries cost far more to merge than
// tiny brown/grey texture variations. A frame-wide budget avoids fixed tiles.
function allocateColorRuns(segments, colors, budget) {
  ...
  const offer = a => {
    ...
    push({ a, b, cost: error - a.error - b.error, w, r, g, blue, square, ink, error });
  };
  ...
  while (count > budget && heap.length) {
    ...
    const next = { w: e.w, r: e.r, g: e.g, b: e.blue, square: e.square, ink: e.ink, error: e.error,
