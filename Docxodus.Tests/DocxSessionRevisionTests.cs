...
public class DocxSessionRevisionTests
{
    ...
    /// view, the comparison engine's input flatten and <c>WmlToHtmlConverter</c>'s accept — pinned on
    ...
    /// <summary>
    /// The guarded prune must read a redline whose inserted note definitions carry an UNMARKED
    /// <c>w:footnoteRef</c> marker run (issue #636's review round). Rejecting such a redline strips
    /// the definition to a marker-only paragraph rather than to nothing — a purely-blockless guard
    ...
    ///
    /// <para>The fixture used to be generated by the legacy comparer, which re-synthesized that marker
    /// run into every inserted definition. It is generated by <see cref="DocxDiff"/> since v11.0.0, so
    /// the test asserts the husk shape is actually PRESENT before relying on it — otherwise a renderer
    /// change would make this pass vacuously, which is exactly the regression it exists to catch.</para>
    /// </summary>
    [Fact]
    public void DS435_StatelessReject_ReadsMarkerScaffoldingAsEmptied()
    {
        var left = File.ReadAllBytes(Path.Combine("../../../../TestFiles/WC", "WC035-Footnote-Before.docx"));
        var right = File.ReadAllBytes(Path.Combine("../../../../TestFiles/WC", "WC035-Footnote-After.docx"));
        var inserting = UserNoteCount(left, "footnote") < UserNoteCount(right, "footnote")
        ...
        AssertHasInsertedMarkerOnlyNoteDefinition(redline);
    ...
    /// <summary>
    /// Assert the redline actually contains an INSERTED footnote definition whose paragraphs carry no
    /// run content beyond the <c>w:footnoteRef</c> marker — the husk shape DS435 exists to exercise.
    /// Without this, a renderer change that stopped emitting the marker run would leave DS435 passing
    /// while testing nothing.
    /// </summary>
    private static void AssertHasInsertedMarkerOnlyNoteDefinition(WmlDocument redline)
    {
        using var stream = new MemoryStream(redline.DocumentByteArray, writable: false);
        using var document = WordprocessingDocument.Open(stream, false);
        var footnotes = document.MainDocumentPart?.FootnotesPart?.GetXDocument().Root;
        Assert.NotNull(footnotes);

        var w = W.w;
        bool hasHusk = footnotes!.Elements(w + "footnote")
            .Where(n => n.Attribute(w + "type") is null)
            .Any(n => n.Descendants(w + "ins").Any()
                   && n.Descendants(w + "r")
                        .Any(r => r.Elements(w + "footnoteRef").Any()
                               && !r.Elements().Any(rc => rc.Name != w + "rPr"
                                                       && rc.Name != w + "footnoteRef")));

        Assert.True(hasHusk,
            "DS435 fixture no longer contains an inserted marker-only footnote definition, so the test " +
            "would pass vacuously. Regenerate the fixture or build the husk shape synthetically.");
    }
