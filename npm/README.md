...
the exact PageMap, renderer fingerprint, warnings, and a schema-v3 render report from that same
sanitized tree.
...
SHA-256-addressed runtime asset graph, and `docxodus/render-report.schema.json` is the current v3
report schema. Frozen v1/v2 and current v3 are also available explicitly as
`docxodus/render-report-v1.schema.json`, `docxodus/render-report-v2.schema.json`, and
`docxodus/render-report-v3.schema.json`.
