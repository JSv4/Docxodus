...
// ============================================================================
// Virtual Pagination / Lazy Loading (Phase 3)
// ============================================================================

/**
 * Options for virtual pagination.
 */
export interface VirtualPaginationOptions {
  /** Scale factor for page rendering (default: 1.0) */
  scale?: number;
  /** Number of pages to buffer around the viewport (default: 3) */
  bufferPages?: number;
  /** CSS class prefix for generated elements (default: "vpage-") */
  cssPrefix?: string;
  /** Background color for the container (default: "#f0f0f0") */
  backgroundColor?: string;
  /** Gap between pages in pixels (default: 16) */
  pageGap?: number;
  /** Callback when a page becomes visible */
  onPageVisible?: (pageNumber: number) => void;
  /** Callback when a page is loaded */
  onPageLoaded?: (pageNumber: number, html: string) => void;
  /** Additional render options */
  renderOptions?: RenderPageRangeOptions;
}

/**
 * State for a single virtual page.
 */
...
/**
 * Result of useVirtualPagination hook.
 */
export interface UseVirtualPaginationResult {
  /** Document metadata (once loaded) */
  metadata: DocumentMetadata | null;
  /** States for all pages */
  pageStates: VirtualPageState[];
  /** Whether metadata is loading */
  isLoading: boolean;
  /** Error from operations */
  error: Error | null;
  /** Total estimated document height in pixels */
  totalHeight: number;
  /** Render a specific page range */
  loadPages: (startPage: number, endPage: number) => Promise<void>;
  /** Check if a page is loaded */
  isPageLoaded: (pageNumber: number) => boolean;
}

/**
 * Hook for managing virtual/lazy pagination of documents.
 *
 * @param document - DOCX file as File object or Uint8Array
 * @param options - Virtual pagination options
 * @param wasmBasePath - Optional custom path to WASM files
 * @returns Virtual pagination state and operations
 *
 * @example
 * ```tsx
 * function LazyDocViewer({ docxFile }: { docxFile: File }) {
 *   const { metadata, pageStates, loadPages, isLoading } = useVirtualPagination(
 *     docxFile,
 *     { scale: 0.8, bufferPages: 2 }
 *   );
 *
 *   if (isLoading) return <div>Loading document info...</div>;
 *   if (!metadata) return null;
 *
 *   return (
 *     <div>
 *       <p>Document has {metadata.estimatedPageCount} estimated pages</p>
 *       {pageStates.map(page => (
 *         <div key={page.pageNumber}>
 *           {page.status === "loaded"
 *             ? <div dangerouslySetInnerHTML={{ __html: page.html! }} />
 *             : <div style={{ height: page.estimatedHeight }}>Loading...</div>
 *           }
 *         </div>
 *       ))}
 *     </div>
...
export function useVirtualPagination(
  ...
  wasmBasePath?: string
): UseVirtualPaginationResult {
  const docxodus = useDocxodus(wasmBasePath);
  const [metadata, setMetadata] = useState<DocumentMetadata | null>(null);
  const [pageStates, setPageStates] = useState<VirtualPageState[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  ...
  const totalHeight = useMemo(() => {
    const gap = options.pageGap ?? 16;
    return pageStates.reduce(
      (sum, page) => sum + page.estimatedHeight * scale + gap,
      0
    );
  }, [pageStates, scale, options.pageGap]);
    ...
    const loadBytes = async () => {
      try {
        if (document instanceof Uint8Array) {
          documentBytesRef.current = document;
        } else {
          const buffer = await document.arrayBuffer();
          if (!cancelled) {
            documentBytesRef.current = new Uint8Array(buffer);
          }
        }
      } catch (err) {
        if (!cancelled) {
  ...
  // Load metadata when document and WASM are ready
  useEffect(() => {
    if (!docxodus.isReady || !document) {
      return;
    }
    ...
    const loadMetadata = async () => {
      ...
      try {
        ...
        const meta = await getDocumentMetadata(document);

        if (cancelled) return;

        setMetadata(meta);

        // Create placeholder states for all pages
        const pageCount = meta.estimatedPageCount;
        const defaultHeight = meta.sections[0]?.contentHeightPt ?? 648;

        const states: VirtualPageState[] = [];
        for (let i = 1; i <= pageCount; i++) {
          // Find which section this page belongs to (estimate)
          let sectionIndex = 0;
          let pagesAccum = 0;
          for (let s = 0; s < meta.sections.length; s++) {
            const section = meta.sections[s];
            const sectionPages = Math.max(1, Math.ceil(
              (section.paragraphCount + section.tableCount * 3) / 25
            ));
            pagesAccum += sectionPages;
            if (i <= pagesAccum) {
              sectionIndex = s;
              break;
            }
          ...
          const section = meta.sections[sectionIndex] ?? meta.sections[0];
          const estimatedHeight = section?.contentHeightPt ?? defaultHeight;
  ...
  }, [docxodus.isReady, document]);

  // Load specific pages
  const loadPages = useCallback(async (startPage: number, endPage: number) => {
    if (!documentBytesRef.current || !docxodus.isReady) {
      return;
    }

    // Mark pages as loading
    setPageStates(prev => prev.map(page => {
      if (page.pageNumber >= startPage && page.pageNumber <= endPage) {
        if (page.status === "placeholder") {
          return { ...page, status: "loading" as const };
        }
      }
      return page;
    }));
    ...
    try {
      ...
      const html = await renderPageRange(
        documentBytesRef.current,
        startPage,
        endPage,
        {
      ...
      // Update page states with loaded HTML
      setPageStates(prev => prev.map(page => {
        if (page.pageNumber >= startPage && page.pageNumber <= endPage) {
          options.onPageLoaded?.(page.pageNumber, html);
          return {
            ...page,
  ...
  }, [docxodus.isReady, scale, options]);

  const isPageLoaded = useCallback((pageNumber: number) => {
    const page = pageStates.find(p => p.pageNumber === pageNumber);
...
/**
 * Props for VirtualPaginatedDocument component.
 */
export interface VirtualPaginatedDocumentProps {
  /** DOCX document as File or Uint8Array */
  document: File | Uint8Array | null;
  /** Scale factor for page rendering (default: 1.0) */
  scale?: number;
  /** Number of pages to buffer around viewport (default: 3) */
  bufferPages?: number;
  /** Gap between pages in pixels (default: 16) */
  pageGap?: number;
  /** Background color for container (default: "#f0f0f0") */
  backgroundColor?: string;
  /** CSS class prefix (default: "vpage-") */
  cssPrefix?: string;
  ...
  /** Callback when a page becomes visible */
  onPageVisible?: (pageNumber: number) => void;
  /** Callback when document metadata is loaded */
  onMetadataLoaded?: (metadata: DocumentMetadata) => void;
  /** Optional custom WASM base path */
  wasmBasePath?: string;
  /** Additional render options */
  renderOptions?: RenderPageRangeOptions;
}
...
/**
 * Virtual scrolling document viewer component.
 *
 * Renders document pages lazily as they come into view, dramatically reducing
 * initial load time and memory usage for large documents.
 *
 * @example
 * ```tsx
 * function DocumentViewer() {
 ...
 *         scale={0.8}
 *         bufferPages={3}
 *         onPageVisible={page => console.log(`Page ${page} visible`)}
 *       />
 *     </div>
 *   );
 * }
 * ```
 */
export function VirtualPaginatedDocument({
  document,
  scale = 1.0,
  bufferPages = 3,
  pageGap = 16,
  ...
  wasmBasePath,
  renderOptions,
}: VirtualPaginatedDocumentProps): ReactElement | null {
  const containerRef = useRef<HTMLDivElement>(null);
  ...
  // Notify when metadata is loaded
  useEffect(() => {
    if (metadata) {
  ...
  useEffect(() => {
    if (!containerRef.current || !metadata || pageStates.length === 0) {
      return;
    }

    // Calculate buffer margin based on average page height
    const avgHeight = pageStates.reduce((sum, p) => sum + p.estimatedHeight, 0) / pageStates.length;
    const bufferPx = bufferPages * avgHeight * scale;
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        entries.forEach(async (entry) => {
          ...
          // Load page if not already loaded/loading
          if (!isPageLoaded(pageNum) && !loadingPagesRef.current.has(pageNum)) {
            loadingPagesRef.current.add(pageNum);
            await loadPages(pageNum, pageNum);
    ...
    const placeholders = containerRef.current.querySelectorAll(`[data-page]`);
    placeholders.forEach(el => observerRef.current?.observe(el));

    return () => {
      observerRef.current?.disconnect();
    };
  }, [metadata, pageStates, bufferPages, scale, loadPages, isPageLoaded, onPageVisible]);
  ...
  // Empty state
  if (!document || !metadata || pageStates.length === 0) {
    return createElement("div", {
      className,
      style: containerStyle,
    }, "No document loaded");
  ...
  return createElement(
    ...
    pageStates.map(page => {
      const section = metadata.sections[page.sectionIndex] ?? metadata.sections[0];
      const width = (section?.pageWidthPt ?? 612) * scale;
      const height = page.estimatedHeight * scale;
      ...
      if (page.status === "loaded" && page.html) {
        return createElement("div", {
          key: page.pageNumber,
          "data-page": page.pageNumber,
          className: `${cssPrefix}page ${cssPrefix}loaded`,
          style: pageStyle,
      ...
      return createElement("div", {
        key: page.pageNumber,
        "data-page": page.pageNumber,
        className: `${cssPrefix}page ${cssPrefix}placeholder`,
        style: {
          ...pageStyle,
          display: "flex",
          alignItems: "center",
      ...
      }, page.status === "loading" ? "Loading..." : `Page ${page.pageNumber}`);
    })
  );
}
