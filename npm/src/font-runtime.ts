...
interface ValidatedResponse {
  response: Omit<FontResolverResponse, "faces" | "outcomes"> & {
    faces: FontResolverFace[];
    outcomes: FontResolverOutcome[];
  };
...
function scalarValues(value: string): number[] {
  ...
  for (const character of value) {
    ...
    result.push(point >= 0xd800 && point <= 0xdfff ? 0xfffd : point);
  }
  return result;
}
...
function collectFontInventory(document: Document, limits: BrowserFontLimits): FontInventory {
  ...
  for (let node = walker.nextNode(); node; node = walker.nextNode()) {
    ...
    for (const point of scalarValues(text)) {
  ...
  const result = ordered.map(({ key, request }, index) => {
...
export function documentFontReadinessCandidates(
  document: Document,
): Array<{ family: string; specification: string; sample: string }> {
  const inventory = collectFontInventory(document, {
    fontFiles: Number.MAX_SAFE_INTEGER,
    fontFileBytes: Number.MAX_SAFE_INTEGER,
    fontTotalBytes: Number.MAX_SAFE_INTEGER,
    fontRequests: Number.MAX_SAFE_INTEGER,
    fontSampleCodePoints: Number.MAX_SAFE_INTEGER,
  });
  return inventory.requests.map((request) => ({
    family: serializeFamilyStack(request.familyStack),
    specification: fontSpecification(request, request.familyStack),
    sample: String.fromCodePoint(...request.sampleCodePoints.slice(0, 4096)) || " ",
  }));
}
...
function positiveNumber(value: unknown, label: string, maximum: number): number {
  if (typeof value !== "number" || !Number.isFinite(value) || value <= 0 || value > maximum) {
    throw new BrowserFontError("invalid_response", `${label} is outside the supported range.`);
  }
  return value;
}
...
async function validateResolverResponse(
  ...
  const record = requireObject(value, "font resolver response");
  exactKeys(record, [
  ...
  if (!Array.isArray(record.faces) || record.faces.length > limits.fontFiles) {
    throw new BrowserFontError("resource_limit", `fontFiles limit exceeded (${Array.isArray(record.faces) ? record.faces.length : "invalid"} > ${limits.fontFiles}).`, "fontFiles");
  }
  const faces = new Map<string, ValidatedFace>();
  let totalBytes = 0;
  for (const [index, value] of record.faces.entries()) {
    const face = requireObject(value, `faces[${index}]`);
    exactKeys(face, [
    ...
    const bytes = canonicalBase64(face.bytesBase64, byteLength, `faces[${index}].bytesBase64`);
    ...
    const evidence = requireObject(face.licenseEvidence, `faces[${index}].licenseEvidence`);
    exactKeys(evidence, ["kind", "identity", "noSubsetting"], `faces[${index}].licenseEvidence`);
    ...
    const resolved: FontResolverFace = {
      id,
      resolvedFamily: normalizeFontFamilyName(shortString(face.resolvedFamily, `faces[${index}].resolvedFamily`)),
      ...
      stretch: positiveNumber(face.stretch, `faces[${index}].stretch`, 1000),
      format,
      mediaType: FORMAT_MEDIA.get(format)! as FontResolverFace["mediaType"],
      byteLength,
      sha256: expectedDigest,
      bytesBase64: face.bytesBase64 as string,
      licenseEvidence: {
        kind: evidence.kind,
  ...
  if (!Array.isArray(record.outcomes) || record.outcomes.length !== requests.length) {
  ...
  for (const [index, value] of record.outcomes.entries()) {
    const outcome = requireObject(value, `outcomes[${index}]`);
    exactKeys(outcome, [
    ...
    const requestedFamily = outcome.requestedFamily === undefined
      ...
      : normalizeFontFamilyName(shortString(outcome.requestedFamily, `outcomes[${index}].requestedFamily`));
    ...
    const resolvedFamily = outcome.resolvedFamily === undefined
      ...
      : normalizeFontFamilyName(shortString(outcome.resolvedFamily, `outcomes[${index}].resolvedFamily`));
    ...
    if (outcome.glyphCoverage === "partial" && (!missingCodePoints || missingCodePoints.length === 0)) {
      throw new BrowserFontError("invalid_response", `outcomes[${index}] must name partially covered code points.`);
    }
    outcomes.push({
      requestId,
      status,
      ...
      ...(missingCodePoints === undefined ? {} : { missingCodePoints }),
    });
  }
...
async function syntheticFamily(
  ...
  const mapping = await digestJson({
    resolvedFamily: fontFamilyKey(resolvedFamily),
    resolverDigest,
  });
...
async function observedFonts(
  ...
  if (document.fonts) {
    await Promise.all(inventory.requests.map(async (request) => {
...
async function configuredFonts(
  ...
  try {
    responseValue = await abortable(resolver({
      schemaVersion: FONT_RESOLVER_SCHEMA_VERSION,
  ...
  await Promise.all(Array.from(validated.faces, async ([id, face]) => {
    pending.add(`face:${id}`);
    try {
      const view = document.defaultView;
  ...
  const syntheticByResolvedFamily = new Map<string, string>();
  for (const outcome of validated.response.outcomes) {
    if (!outcome.faceId || !successfulFaces.has(outcome.faceId) || !outcome.resolvedFamily) continue;
    const resolvedKey = fontFamilyKey(outcome.resolvedFamily);
    let synthetic = syntheticByResolvedFamily.get(resolvedKey);
    if (!synthetic) {
      synthetic = await syntheticFamily(resolverDigest, outcome.resolvedFamily);
      syntheticByResolvedFamily.set(resolvedKey, synthetic);
    }
