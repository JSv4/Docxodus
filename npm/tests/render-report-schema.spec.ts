...
const schemaPath = resolve(testDirectory, '../../docs/schemas/render-report-v1.schema.json');
...
test.describe('render-report v1 font schema', () => {
  test('keeps the complete and failed report branches wired to the same font definitions', () => {
    ...
    expect(objectProperties(complete).fontIdentity).toEqual({
      $ref: '#/$defs/baseProperties/fontIdentity',
    });
